    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addSkillModal">Add Skill</h5>
            ...
            <div class="modal-body">
                <form class="form-horizontal form-bordered add-form" method="POST" action="{{ route('skill.add') }}">
                    @csrf

                    <div class="form-group required">
                        <label class="col-md-3 control-label form-label" for="skillName">Skill</label>
                        <div class="col-md-6">
                            <input type="text" name="skillName" id="skillName" class="form-control" required tabindex="1" value="{{ old('skillName') }}">
                        </div>
                    </div>
                        ...
                        <div class="col-md-6">
                            <select name="skillCategory" id="skillCategory" class="form-control" required tabindex="1">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->category_name }}" {{ old('skillCategory') == $category->category_name ? 'selected' : '' }}>
                    ...
                    <div class="form-group required">
                        ...
                        <div class="col-md-6">
                            <select name="skillValue" id="skillValue" class="form-control">
                                ...
                                @for ($i = 1; $i <= 10; $i++)
                                    <option value="{{ $i }}" {{ old('skillValue') == $i ? 'selected' : '' }}>{{ $i }}</option>
                                @endfor
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-group required">
                        ...
                        <div class="col-md-6">
                            <textarea name="skillFrom" id="skillFrom" class="form-control" tabindex="9">{{ old('skillFrom') }}</textarea>
                        </div>
                    </div>
